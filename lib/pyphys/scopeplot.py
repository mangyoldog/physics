...
#def merge_figs(figures, axes, figsize):
#  fig3, ax3 = plt.subplots(sharex=True, sharey=True, figsize=(6,4))
#  lines = []
#  for f, fig in enumerate(figures):
...
def plot_mTrace(dataset, time_scale = 1., channel='loadv', channel_scale=1., 
...
#def plot_multiTrace_dt(dataset, time_scale = 1., channel='loadv', channel_scale=1., 
#    alpha=0.8, colors=cs.clist(), labels=None, center=False):
#  """ Returns a figure of osrs in the order of the dataset provided.
#  requres time/chgv/loadv dataset. Note time does not currently work but 
#  time scale can be used to adjust the time bins"""
#  fig = plt.figure(figsize=(5,4))
#  Plot = fig.add_subplot(111)
...
#  if center:
#    time = time - np.median(time)
#  lines.append(ax1.plot(time,loadv, color=colors[0], label='Load', alpha=alpha))
#  ax2 = ax1.twinx()
#  lines.append(ax2.plot(time,chgv, color=colors[1], label='Charge', alpha=alpha))
